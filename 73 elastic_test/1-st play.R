...
jl <- as.Node(m)
list.names(m)
rl <- list.select(m, type=`_type`, timestamp=`_source`$`@timestamp`) # так достается с помощью rlist
rl <- list.select(m, start_time=`_source`$`start_time`, 
                  src_ip=`_source`$source$ip,
                  src_port=`_source`$source$port,
                  bytes_sent=`_source`$source$stats$net_bytes_total) # так достается с помощью rlist
