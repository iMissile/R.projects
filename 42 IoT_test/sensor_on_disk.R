...
# дл€ сходимости задачи генерируем также зафиксированные точки на окружности
...
while (max.force > 0.5) {
  # провер€ем, что точки не ушли из окружности единичного радиуса
...
# 1. ”бираем фиксированные точки
df.calc <- filter(dfs, !fixed)
# 2. ¬ыбираем в качестве начальной точки датчик, максимально близко расположенный к краю пол€
...
rx.geo <- sqrt((x0.geo - 38.662979)^2 + (y0.geo - 54.860416)^2) # права€ горизонтальна€ точка
ry.geo <- sqrt((x0.geo - 38.655267)^2 + (y0.geo - 54.865068)^2) # верхн€€ вертикальна€ точка

# «оологическа€ 2
x0.geo <- 37.58022 # Longitude
y0.geo <- 55.76559 # Latitude
rx.geo <- sqrt((0.01)^2) # права€ горизонтальна€ точка
ry.geo <- sqrt((0.01)^2) # верхн€€ вертикальна€ точка


# а еще не забываем, что надо преобразовывать координаты из метров в градусы
# ¬ычисление рассто€ни€ и начального азимута между двум€ точками на сфере
