...
igraph::set_vertex_attribute(g, "ip_addr", stringr::str_c("192.168.1.", sample(1:254, s, replace=FALSE))) # or %v%
```{r}
...
s <- network.size(net)
s
network::set.vertex.attribute(net, "ip_addr", stringr::str_c("192.168.1.", sample(1:254, s, replace=FALSE))) # or %v% Letter the vertices
...
gp <-
  ...
  geom_nodelabel_repel(aes(label=ip_addr), fontface = "bold", box.padding=unit(2, "lines"), segment.colour="red", segment.size=1) +
  # theme_blank()
  theme_minimal() +
...
png(filename="plot_w_cairo.png", type="cairo", #pointsize=24, 
    units="cm", height=10, width=15, res=300, antialias="default")
gp
dev.off()
```
