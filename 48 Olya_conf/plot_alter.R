...
# пробегаемс€ по строчкам data.frame
...
pp0 <- pp.base + 
  scale_size_continuous(range = c(1.5, 4.5)) + # управл€ем диапазоном размера точек
  scale_x_continuous(breaks = pretty_breaks(n = 20))
  
pp <- pp.base +
  scale_size_continuous(range = c(0.5, 3)) + # управл€ем диапазоном размера точек
