...
# попробуем отобразить графики. предварительна€ подготовка ================
df %<>%
...
# а теперь выберем из исходного материала только данные, касающиес€ этих “ќѕ N
df2 <- df %>%
...
gg <- ggplot(df3 %>% top_n(10, rur), aes(x=fct_reorder(CP, rur, .desc=TRUE), y=rur)) +
  ...
  xlab(" онтент-провайдер") +
  ylab("ƒенежки за период анализа, руб.") +
  ggtitle("јнализ денежных потоков")  
